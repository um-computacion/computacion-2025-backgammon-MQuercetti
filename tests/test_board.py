...
class TestBoard(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.board.player1, self.p1)
        self.assertEqual(self.board.player2, self.p2)
        self.assertIsNone(self.board.winner)
        self.assertEqual(self.board.current_player, self.p1)
        ...
        self.assertEqual(self.board.bar[self.p2], [])
        self.assertEqual(self.board.off_board[self.p1], 0)
        self.assertEqual(self.board.off_board[self.p2], 0)

    def test_get_point(self):
        """Test getting point checkers."""
        point = self.board.get_point(23)
        self.assertEqual(len(point), 2)
        self.assertEqual(point[0].owner, self.p1)
    ...
    def test_is_valid_move_blocked(self):
        ...
        self.board.points[22] = [Checkers(self.p2), Checkers(self.p2)]
    ...
    def test_is_valid_move_bar_priority_white(self):
        ...
        # Put white on bar
        self.board.bar[self.p1].append(Checkers(self.p1))
    ...
    def test_move_piece_normal(self):
        ...
        initial_22 = len(self.board.points[22])
        self.board.move_piece(23, 1, self.p1)
        self.assertEqual(len(self.board.points[23]), initial_23 - 1)
        self.assertEqual(len(self.board.points[22]), initial_22 + 1)

    def test_move_piece_hit(self):
        """Test hit: Send single opponent to bar."""
        # Put 1 black on 22
        self.board.points[22] = [Checkers(self.p2)]
        initial_bar_black = len(self.board.bar[self.p2])
        self.board.move_piece(23, 1, self.p1)
        self.assertEqual(len(self.board.points[22]), 1)  # White now on 22
        self.assertEqual(len(self.board.bar[self.p2]), initial_bar_black + 1)  # Black hit
    ...
    def test_move_piece_bear_off(self):
        ...
        self.board.points[0] = [Checkers(self.p1)]  # White on 0
        self.board.move_piece(0, 1, self.p1)  # Bear-off
        self.assertEqual(self.board.off_board[self.p1], 1)
    ...
    def test_ai_play_turn(self, mock_roll):
        ...
        self.board.ai.play_turn([3, 4])
        # AI should have moved if possible
        self.assertLess(len(self.board.points[0]), initial_0)  # Moved
